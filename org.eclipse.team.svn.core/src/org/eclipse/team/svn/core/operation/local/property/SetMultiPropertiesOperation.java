		...
		for (int i = 0; i < properties.length && !monitor.isCanceled(); i++) {
			...
			this.protectStep(new IUnprotectedOperation() {
				public void run(IProgressMonitor monitor) throws Exception {
                	proxy.propertySet(wcPath, property.name, property.data == null ? property.value.getBytes() : property.data, false, new SVNProgressMonitor(SetMultiPropertiesOperation.this, monitor, null));
				}
