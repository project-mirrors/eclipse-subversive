...
public class JavaHLMergeOperation extends AbstractWorkingCopyOperation {
	...
	protected void doMerge(IResource resource, IRepositoryResource from1, IRepositoryResource from2, IProgressMonitor monitor) throws Exception {
		...
		try {
			...
			proxy.merge(
				SVNUtility.encodeURL(from1.getUrl()), from1.getSelectedRevision(),
				SVNUtility.encodeURL(from2.getUrl()), from2.getSelectedRevision(),
				wcPath, false, true, this.ignoreAncestry, this.dryRun, new MergeProgressMonitor(this, monitor, null));
		}
		finally {
