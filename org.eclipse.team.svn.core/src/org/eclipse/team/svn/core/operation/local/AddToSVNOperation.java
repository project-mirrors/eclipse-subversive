...
public class AddToSVNOperation extends AbstractWorkingCopyOperation {
	...
	public static void removeFromParentIgnore(ISVNClientWrapper proxy, String parentPath, String name) throws Exception {
		PropertyData data = proxy.propertyGet(parentPath, PropertyData.IGNORE, new SVNNullProgressMonitor());
		...
		{
			proxy.propertySet(parentPath, PropertyData.IGNORE, ignoreValue, false, new SVNNullProgressMonitor());
		}
		else
		{
			proxy.propertyRemove(parentPath, PropertyData.IGNORE, false, new SVNNullProgressMonitor());
		}
	}
	...
	protected void doAdd(IResource current, ISVNClientWrapper proxy, IProgressMonitor monitor) throws Exception {
		...
		proxy.add(wcPath, AddToSVNOperation.this.isRecursive, new SVNProgressMonitor(AddToSVNOperation.this, monitor, null));
	}
