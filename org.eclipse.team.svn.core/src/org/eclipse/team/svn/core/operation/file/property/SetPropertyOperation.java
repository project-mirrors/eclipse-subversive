				...
				public void run(IProgressMonitor monitor) throws Exception {
					for (int i = 0; i < SetPropertyOperation.this.propertyData.length && !monitor.isCanceled(); i++) {
					    ...
					    SetPropertyOperation.this.protectStep(new IUnprotectedOperation() {
	                        public void run(IProgressMonitor monitor) throws Exception {
	        					proxy.propertySet(current.getAbsolutePath(), property.name, property.data, SetPropertyOperation.this.isRecursive, new SVNProgressMonitor(SetPropertyOperation.this, monitor, null));
	                        }
