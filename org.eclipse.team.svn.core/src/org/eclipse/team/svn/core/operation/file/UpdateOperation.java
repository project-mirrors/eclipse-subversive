		...
		for (Iterator it = wc2Resources.entrySet().iterator(); it.hasNext() && !monitor.isCanceled(); ) {
			...
			this.protectStep(new IUnprotectedOperation() {
				public void run(IProgressMonitor monitor) throws Exception {
					proxy.update(paths, UpdateOperation.this.selectedRevision, Depth.unknownOrFiles(true), false, false, new ConflictDetectionProgressMonitor(UpdateOperation.this, monitor, null));
				}
