...
//		proxy.list(SVNUtility.encodeURL(parentUrl), this.getSelectedRevision(), this.getPegRevision(), false, true);
		Info2 []infos = proxy.info2(SVNUtility.encodeURL(url), this.getSelectedRevision(), this.getPegRevision(), false, new SVNNullProgressMonitor());
		if (infos != null && infos.length > 0 && infos[0].lastChangedRevision != Revision.SVN_INVALID_REVNUM) {
			...
			//FIXME read hasProps value using list for parent node...
			this.setInfo(new IRepositoryResource.Info(infos[0].lock, 0, infos[0].lastChangedAuthor, new Date(infos[0].lastChangedDate), false/*infos[0].getHasProps()*/));
		}
	}
