...
public class HistoryViewImpl {
	...
	public void createPartControl(Composite parent) {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				...
				tAction.setEnabled(tSelection.size() == 2 && (HistoryViewImpl.this.repositoryResource instanceof IRepositoryFile ||
						CoreExtensionsManager.instance().getSVNClientWrapperFactory().isCompareFoldersAllowed()));
				if (tSelection.size() == 1) {
					...
					tAction.setEnabled(tSelection.size() == 1 && (HistoryViewImpl.this.repositoryResource instanceof IRepositoryFile ||
							CoreExtensionsManager.instance().getSVNClientWrapperFactory().isCompareFoldersAllowed()));
				}
	...
	public void refresh() {
		...
		if (this.wcResource == null) {
			this.showHistory(this.repositoryResource, false);
		}
		else {
			this.showHistory(this.wcResource, false);			
		}
...
//						HistoryViewImpl.this.viewInfoProvider.setDescription(HistoryViewImpl.this.getResourceLabel());
	...
	protected void addRevisionLinks(IStructuredSelection tSelection) {
		...
		for (Iterator iter = tSelection.iterator(); iter.hasNext();) {
			...
			selectedRevisions.add(new Long(item.revision));
		}
		
		CompositeOperation op = new CompositeOperation("Operation.HAddSelectedRevision");
		for (Iterator iter = selectedRevisions.iterator(); iter.hasNext();) {
			Long revision = (Long)iter.next();
			AbstractActionOperation mainOp = new AddRevisionLinkOperation(this.repositoryResource, revision.longValue());
			op.add(mainOp);
		}
	...
	protected void handleDoubleClick(Object item, boolean doubleClick) {
		...
		if ((this.options & HistoryViewImpl.COMPARE_MODE) != 0 && doubleClick && (this.repositoryResource instanceof IRepositoryFile ||
				CoreExtensionsManager.instance().getSVNClientWrapperFactory().isCompareFoldersAllowed())) {
	...
	protected void addMenuItem(MenuManager menuManager, String label, final AbstractRepositoryTeamAction action) {
		...
		if (tSelection.size() == 1) {
			resourceSelection = new StructuredSelection(new RepositoryFile(this.getResourceForSelectedRevision(tSelection.getFirstElement())));
		}
		else {
