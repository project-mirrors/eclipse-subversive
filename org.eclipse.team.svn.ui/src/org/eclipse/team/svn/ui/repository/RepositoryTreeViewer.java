...
public class RepositoryTreeViewer extends TreeViewer {
	...
	public void refresh(final Object element, final IRefreshVisitor visitor) {
		this.getControl().getDisplay().syncExec(new Runnable() {
			public void run() {
				TreeItem []nodes = RepositoryTreeViewer.this.getIdenticalNodes(element, false);
				if (nodes != null && nodes.length != 0) {
...
//		this.updateChildren(widget, element, null);
	...
	protected void handleDoubleClick(IStructuredSelection selection) {
    	...
    	if (node instanceof IParentTreeNode) {
			this.setExpandedState(node, !this.getExpandedState(node));
    	}
	}
