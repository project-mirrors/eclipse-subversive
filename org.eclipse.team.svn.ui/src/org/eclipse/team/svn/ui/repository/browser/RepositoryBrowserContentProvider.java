...
public class RepositoryBrowserContentProvider extends BaseWorkbenchContentProvider {
	...
	public Object[] getChildren(Object parentElement) {
		...
		if (adapter instanceof RepositoryFolder) {
			Object[] children = ((RepositoryFolder)adapter).peekChildren(parentElement);
			if (this.isRepositoryLocationRoot((IResourceTreeNode)adapter)) {
				return children;
			}
	...
	public boolean isRepositoryLocationRoot(IResourceTreeNode node) {
		boolean retVal = false;
		IRepositoryResource resource = node.getRepositoryResource();
		IRepositoryLocation location = resource.getRepositoryLocation();
		String resourceURL = resource.getUrl();
		String locationURL = location.getUrl();
		if (resourceURL.equals(locationURL)) {
			retVal = true; 
		}
		return retVal;
	}
