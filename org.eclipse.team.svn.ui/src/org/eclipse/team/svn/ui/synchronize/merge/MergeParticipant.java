	...
	protected class MergeLabelDecorator extends LabelDecorator {
		...
		public Image decorateImage(Image image, Object element) {
			...
			if (info != null && (info.getKind() & LabelDecorator.CONFLICTING_REPLACEMENT_MASK) == LabelDecorator.CONFLICTING_REPLACEMENT_MASK) {
		        String wcKind = info.getWCStatus() == null ? IStateFilter.ST_NOTEXISTS : info.getWCStatus().getStatus();
		        int mask = info.getWCStatus() == null ? 0 : info.getWCStatus().getChangeMask();
		        if (IStateFilter.SF_PREREPLACEDREPLACED.accept(info.getLocal(), wcKind, mask)) {
