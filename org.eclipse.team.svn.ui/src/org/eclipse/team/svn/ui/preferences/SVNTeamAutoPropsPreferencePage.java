 ...
 *
 */
public class SVNTeamAutoPropsPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	protected void performDefaults() {
		...
		this.populateTable(SVNTeamAutoPropsPreferencePage.loadProperties(store, true));
	}
	...
	public void saveProperties() {
		...
		for (int i = 0; i < propsCount; i++) {
			SVNTeamAutoPropsPreferencePage.AutoProperty property =
				(SVNTeamAutoPropsPreferencePage.AutoProperty)this.tableViewer.getElementAt(i);
			props[3 * i] = property.enabled ? "1" : "0";
			props[3 * i + 1] = property.fileName;
			props[3 * i + 2] = property.properties;
		}
		IPreferenceStore store = this.getPreferenceStore();
		SVNTeamPreferences.setAutoPropertiesList(store,
				SVNTeamPreferences.AUTO_PROPERTIES_LIST_NAME,
				FileUtility.encodeArrayToString(props));
	...
	public void populateTable() {
		IPreferenceStore store = this.getPreferenceStore();
		Object[] properties = SVNTeamAutoPropsPreferencePage.loadProperties(store);
		this.populateTable(properties);
	}
	
	public void populateTable(Object[] items) {
		for (int i = 0; i < items.length; i++) {
			this.tableViewer.add(items[i]);
			this.tableViewer.setChecked(items[i], 
					((SVNTeamAutoPropsPreferencePage.AutoProperty)items[i]).enabled);
		}
	}
	
	public static Object[] loadProperties(IPreferenceStore store) {
		return SVNTeamAutoPropsPreferencePage.loadProperties(store, false);
	}
	
	public static Object[] loadProperties(IPreferenceStore store, boolean useDefault) {
		ArrayList propsList = new ArrayList();
		String encodedProps;
		if (useDefault) {
			encodedProps = SVNTeamPreferences.AUTO_PROPERTIES_LIST_DEFAULT;
		}
		else {
			encodedProps = SVNTeamPreferences.getAutoPropertiesList(store, SVNTeamPreferences.AUTO_PROPERTIES_LIST_NAME);
		}
