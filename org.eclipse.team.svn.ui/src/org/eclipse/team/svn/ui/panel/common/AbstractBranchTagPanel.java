...
public abstract class AbstractBranchTagPanel extends AbstractDialogPanel {
    ...
    public AbstractBranchTagPanel(IRepositoryRoot root, boolean showStartsWith, Set existingNames, String nationalizationId, String historyName, IResource[] resources) {
		...
		IResource[] addedResources = FileUtility.getResourcesRecursive(resources,
				new IStateFilter() {
		...
		this.disableSwitch = addedResources.length > 0;
        ...
        if (this.disableSwitch) {
        	this.defaultMessage += " " + SVNTeamUIPlugin.instance().getResource(this.nationalizationId + ".Warning");
        }
		...
		this.considerStructure = 
			root.getRepositoryLocation().isStructureEnabled() &&
    ...
    public void createControls(Composite parent) {
		...
		if (this.startsWith) {
			...
			this.startWithCheck.setEnabled(!this.disableSwitch);
		...
		if (!this.considerStructure) {
			...
			browse.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
				    RepositoryBrowsingPanel panel = new RepositoryBrowsingPanel(SVNTeamUIPlugin.instance().getResource(AbstractBranchTagPanel.this.nationalizationId + ".SelectionProposal"), AbstractBranchTagPanel.this.root.getRepositoryLocation());
			...
			browse.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
				    RepositoryBrowsingPanel panel = new RepositoryBrowsingPanel(SVNTeamUIPlugin.instance().getResource(AbstractBranchTagPanel.this.nationalizationId + ".SelectionProposal"), AbstractBranchTagPanel.this.root);
					DefaultDialog browser = new DefaultDialog(AbstractBranchTagPanel.this.manager.getShell(), panel);
					if (browser.open() == 0) {
						IRepositoryResource selected = panel.getSelectedResource();
						if (selected != null) {
    ...
    protected void setMessage(int level, String message) {
    	if (level == IDialogManager.LEVEL_OK && this.disableSwitch) {
    		level = IDialogManager.LEVEL_WARNING;
    	}
