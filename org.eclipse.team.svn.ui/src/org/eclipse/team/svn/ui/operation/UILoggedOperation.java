...
public class UILoggedOperation extends LoggedOperation {
    ...
    public static OperationErrorInfo formatMessage(IStatus status, boolean allowsCancelled) {
        if (!status.isMultiStatus()) {
        	...
        	if (!allowsCancelled && (ex instanceof ClientWrapperCancelException || ex instanceof ActivityCancelledException || ex instanceof OperationCanceledException) || 
        ...
        for (int i = 0; i < children.length; i++) {
        	...
        	if (!allowsCancelled && (exception instanceof ClientWrapperCancelException || exception instanceof ActivityCancelledException || exception instanceof OperationCanceledException) || 
